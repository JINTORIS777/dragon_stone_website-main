...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        if ($user) {
            ...
            $_SESSION['login_success'] = true; // Flag for success message
            
            // Ensure user has a subscription (create free if none exists)
            $userSubscription = getUserSubscription($user['id']);
            if (!$userSubscription) {
                $subscriptionResult = createUserSubscription($user['id'], 'free');
                if ($subscriptionResult['success']) {
                    $_SESSION['subscription_created'] = true;
                }
            }
            
            // Build redirect URL - default to account page
            $redirect = $next ?: 'account.php';
            $qs = [];
            if ($product) $qs[] = 'product=' . urlencode($product);
            if ($price) $qs[] = 'price=' . urlencode($price);
            if (!empty($qs)) {
                $redirect .= (strpos($redirect, '?') === false ? '?' : '&') . implode('&', $qs);
            }
        ...
        <section class="login-section">
            <div class="login-card">
                ...
                <form id="login-form" method="post">
                    ...
                    <a href="register.php?next=<?php echo urlencode($next); ?>&product=<?php echo urlencode($product); ?>&price=<?php echo urlencode($price); ?>" class="btn register-link">DON'T HAVE AN ACCOUNT? REGISTER</a>
                    <div id="login-alert" class="alert <?php echo $login_error ? 'alert-danger d-block' : 'd-none'; ?>"><?php echo htmlspecialchars($login_error); ?></div>
                </form>
            </div>
        </section>
